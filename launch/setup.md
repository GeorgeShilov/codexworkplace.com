...
- for this project, the domain is `atenwallet.com`
...
- Railway project: `codexworkplace.com`
- Railway service: `codexworkplace.com`
- Railway temporary domain: `https://codexworkplacecom-production.up.railway.app`
- Custom domain: `codexworkplace.com`
...
```bash
railway init -n codexworkplace.com -w <workspace-id> --json
railway add --service codexworkplace.com --json
railway up --service codexworkplace.com --detach --verbose
railway domain --service codexworkplace.com --json
railway variable set SITE_URL=https://codexworkplace.com --service codexworkplace.com --environment production --skip-deploys
```

If `railway domain codexworkplace.com ...` returns `Unauthorized` even after `railway login`, use the Railway dashboard or Railway GraphQL API to create the custom domain. In this run, the CLI was intermittently authenticated for `status` but unauthorized for `domain`.
...
Railway gave these records for `codexworkplace.com`:
...
Zone > DNS > Edit
Zone > Zone > Read
Zone Resources: Include > Specific zone > codexworkplace.com
```
...
const zoneName = "codexworkplace.com";
...
```bash
curl -s "https://cloudflare-dns.com/dns-query?name=codexworkplace.com&type=A" \
  -H "accept: application/dns-json"

curl -s "https://cloudflare-dns.com/dns-query?name=_railway-verify.codexworkplace.com&type=TXT" \
  -H "accept: application/dns-json"
```
